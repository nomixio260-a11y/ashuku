...
- **CSV/区切りテキストの列指向(columnar)変換**: 矩形 CSV は「行 = 異種
  ...
  最適コーデックを best-of で選択**——単調な数値列は **delta(隣接差)**、列挙・
  ステータス・ホスト名など低カーディナリティ列は **dict(パレット+ID)**、
  高カーディナリティ列は raw。実測で行指向比 **-32.7%**(従来 -25% から改善。
  RESEARCH.md §4.30 / §4.43)。転置は split/join の完全な逆変換で、矩形でありさえすれば引用符・
...
- **JSONL(構造化ログ)の骨格分離+列指向変換**: 改行区切り JSON(構造化ログの
  ...
  **-33.5%**(従来 -27% から改善。RESEARCH.md §4.31 / §4.43)。**値の中身(エスケープ・数値表現・Unicode)を
...
| ログ・テキスト | 5〜45倍 |
| CSV/区切りテキスト(矩形) | **6〜60倍**(列指向転置+列別 dict/delta/raw で行指向比 −32.7%、ビット一致復元) |
| JSONL(構造化ログ・同一スキーマ) | **6〜60倍**(骨格分離+列別 dict/delta/raw で行指向比 −33.5%、ビット一致復元) |
