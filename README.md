...
- **リージョン(ソリッド)圧縮**: チャンクを独立に圧縮するとチャンクをまたぐ
  ...
  87%)。近いもの同士の並べ替えには **min-hash 署名(共有語彙=Jaccard)**を
  使い、同一スキーマ・異値のチャンク(同フォーマットの別ログ等)を高確率で
  近接させます(近重複検出用の max ベース特徴では捉えられなかった信号。
  異種混在の小ファイルで隣接率 16%→80%、リージョン −4.5%。RESEARCH.md §4.48)。
  読み出しはリージョン単位でキャッシュされ、削除・低生存率リージョンの回収も自動です。
- **CSV/区切りテキストの列指向(columnar)変換**: 矩形 CSV は「行 = 異種
  ...
  19桁 epoch も対応)**、間隔が漸増する/ジッタの大きい時系列は **delta2(二階差分)**、
  漸増する**固定小数列(気温・価格・センサ値)**は
  スケール整数の delta(相関ゲート付き。乱数小数は raw に退避)、列挙・
  ステータス・ホスト名など低カーディナリティ列は
  **dict(パレット+ID、ID は ASCII/固定幅二進の best-of)**、トレース/リクエスト
  ID・ハッシュ等の**固定幅 hex 列**は **nibble パック(ASCII 8→4bit/文字、乱数hexの
  エントロピー床まで)**、単調 hex カウンタ・**ゼロ埋め連番**は整数 delta
  (dict が弾く全ユニーク列・parseCanonInt 非対象を取り込み。RESEARCH §4.55)、
  高カーディナリティ列は raw。CRLF(`\r\n`)終端 CSV も検出して行区切りを剥がします。区切りは
  **`,` / タブ(TSV) / `;`(欧州 CSV・ログ)を自動検出**——従来 `,` 以外は素通し
  でしたが、TSV 実測で **−41%**(それまで 0 だったクラス)。実測で
  行指向比 **-31〜50%**(センサ/ログ系ほど大。RESEARCH.md §4.30 / §4.43 / §4.45 / §4.53)。
...
- **precompression(gzip / 連結gzip / 生zlib)**: zlib 産の deflate 系ストリームは
  ...
  **ZIP コンテナ**(docx/xlsx/jar 等。zlib 産 deflate メンバーを展開し、
  ...
  なります(他機能は影響なし)。
  さらに **再帰 precompression**: `.log.gz` / `.csv.gz` / `.json.gz` は展開
  データ(素のログ/CSV/JSON)に**内側でさらに列指向変換を重ねます**。復元は
  内側(列指向)→外側(gzip 再圧縮)の順で、両段ともバイト一致検証済みなので
  合成しても可逆。実測: zlib 産 nginx ログの gzip(252KB、展開 1.8MB)を
  **gzip ファイルより 45% 小さい 139KB で保存**(RESEARCH.md §4.50)。圧縮済み
  ログ・表アーカイブという定番ワークロードに列指向の利得が届きます。
...
| `-chunk-avg` | `0`(=256KiB) | 平均チャンクサイズ(バイト)。初回起動時のみ有効。小さいほど dedup が細かく効くがメタデータ増。dedup 極大化は 64KiB 明示指定(RESEARCH §4.54) |
