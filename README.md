...
- **PCM 音声(WAV)の可逆圧縮**: 非圧縮 WAV は従来そのまま(1.0倍)保存されて
  いましたが、PCM サンプルは時間的に強く相関するので、FLAC/Shorten と同じ
  **固定線形予測**(特許フリー)をチャンネルごとにかけて残差にし、バイト平面
  に並べて zstd/brotli で圧縮します。予測↔逆予測は整数環の完全な全単射なので
  **原理的に往復不能があり得ない**最も安全な変換です(縮む時だけ採用・雑音は
  素通し)。**WAV(リトルエンディアン)と AIFF(ビッグエンディアン)の両方**に
  対応。実測: 16bit 音声で −25〜−36%、低エントロピー音源はさらに大。
- **BMP(非圧縮ビットマップ)の可逆圧縮**: 生ピクセルに **PNG と同じ行予測
  フィルタ**(None/Sub/Up/Average/Paeth を行ごとに自動選択)をかけて空間相関
  を除き、zstd/brotli で圧縮します。フィルタ↔逆フィルタは 256 剰余環の全単射
  で最も安全な変換です。実測: 写真調 BMP で **−51%**。
...
| WAV(非圧縮 PCM 音声) | **1.3〜2倍以上**(16bit で −25〜−36%、低エントロピー音源はさらに大) |
