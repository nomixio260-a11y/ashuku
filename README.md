...
- **JPEG 可逆再圧縮(画像)**: baseline JPEG は Huffman を復号して量子化 DCT
  係数に戻し、係数を並べ替えて zstd/重複排除/デルタの対象にします
  (lepton/packJPG と同じ原理の**純Go実装・cgo不要**)。ビット一致で復元でき、
  **係数平面を zstd-19 した見込みが元より小さい場合のみ採用**(単画像で決して
  膨らませません)。実測: スクショ -73%、実写真 -3.5〜-7%、合成写真で
  ストア経路 -30%。似た画像(バースト・再保存・サムネイル・編集版)は
  係数が似るため重複排除・デルタが効くようになります(JPEG のままでは
  1バイトの違いで全ストリームが変わり、何も効きません)。プログレッシブ
  JPEG・動画は対象外(動画は既に理論限界近くで、可逆に縮む余地がありません)。
