...
- **gzip precompression**: zlib 産の gzip(Python/Java/nginx 等、実世界の gzip の
  最大勢力)は「展開データ+再構成レシピ(ヘッダ原文と圧縮レベル)」に分解して
  保存され、**展開データに対して重複排除・類似デルタ・zstd が効く**ようになります
  (gzip のままでは中身が少し違うだけで全バイトが変わり、何も効きません)。
  分解は保存時にビット一致を検証してからのみ採用され、zlib 産でない gzip
  (GNU gzip / Go 等)は安全に素通しされます。復元時も SHA-256 で最終検証します。
