// FastCDC のコア実装(依存を持たない自前実装)。
//
// 以前は github.com/jotfs/fastcdc-go を使っていたが、同ライブラリの
// NewChunker はパッケージ大域の gear テーブルを `table[i] ^= Seed` で
// **その場書き換え**する実装で、複数アップロードを並行にチャンク化すると
// この大域への書き込みと nextChunk の読み出しがデータ競合を起こす
// (Seed=0 なので値は変わらず無害だが Go メモリモデル上は未定義動作で
// -race も落ちる)。そこで境界が完全に一致するよう同一アルゴリズム・同一
// gear テーブルを移植し、テーブルを**読み取り専用**にして競合を除いた。
// アルゴリズム: FastCDC(正規化チャンキング、Normalization=2)。
package chunker
...
// gearTable は FastCDC ローリングハッシュの 256 エントリ gear テーブル。
// 初期化後は一切書き換えない(読み取り専用=並行チャンク化でも競合なし)。
...
// cdc は1ストリーム分のチャンカー状態(fastcdc-go と同一の意味論)。
type cdc struct {
	minSize, maxSize, normSize int
	maskS, maskL               uint64
	rd                         io.Reader
	buf                        []byte
	cursor                     int
...
// newCDC は avgSize から fastcdc-go と同一のパラメータでチャンカーを作る
// (MinSize=avg/4, MaxSize=avg*4, Normalization=2, BufSize=MaxSize*2, Seed=0)。
func newCDC(rd io.Reader, avgSize int) *cdc {
	minS := avgSize / 4
	maxS := avgSize * 4
	bufSize := maxS * 2
	bits := int(math.Round(math.Log2(float64(avgSize))))
	smallBits := bits + 2 // Normalization=2
	largeBits := bits - 2
	return &cdc{
		minSize:  minS,
		maxSize:  maxS,
		normSize: avgSize,
		maskS:    (1 << uint(smallBits)) - 1,
		maskL:    (1 << uint(largeBits)) - 1,
		rd:       rd,
...
// next は次のチャンクのバイト列を返す。終端では (nil, io.EOF)。
func (c *cdc) next() ([]byte, error) {
	...
	length := c.nextChunk(c.buf[c.cursor:])
	data := c.buf[c.cursor : c.cursor+length]
	c.cursor += length
	return data, nil
}

func (c *cdc) nextChunk(data []byte) int {
	fp := uint64(0)
	i := c.minSize
