...
type FileManifest struct {
	...
	// PrecompAVI は AVI/MJPEG の再構成レシピ。
	PrecompAVI *precomp.MJPEGRecipe `json:"precomp_avi,omitempty"`
	// PrecompWAV は WAV(PCM)の再構成レシピ。
	...
	// PrecompHEVC は HEVC(Annex B)の再構成レシピ。
	...
	// PrecompAAC は AAC(ADTS)の再構成レシピ。
	PrecompAAC *precomp.AACRecipe `json:"precomp_aac,omitempty"`
...
const (
	...
	// EncodingAVIV1 は AVI/MJPEG の各 JPEG フレームを分解して保存。
	...
	// EncodingHEVCV1 は HEVC の I スライス CABAC を再符号化して保存。
